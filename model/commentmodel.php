...
class CommentModel extends Model{
	...
	/*public function definitiveComment($idPost){
		$sql = "SELECT 
			author AS `{{ author }}`, 
			comment AS `{{ content }}`, 
			DATE_FORMAT(date, '%d/%m/%Y') AS `{{ published }}`,
			...
			FROM `comments` 
			WHERE `idPost` = '$idPost' AND `state`= 3 ORDER BY date DESC";
	...
	public function showedComment(){
		
		$sql = "SELECT 
			author AS `{{ reportauthor }}`, 
			comment AS `{{ reportcomment }}`, 
			DATE_FORMAT(date, '%d/%m/%Y') AS `{{ reportpublished }}`,
			idPost AS `{{ reportidPost }}`,
			idComment AS `{{ idComment }}`
			
			FROM `comments` 
			WHERE `state`= (1 OR 3) ORDER BY date DESC";
...
//die(var_dump($result));
	...
	public function validateNewComment($idComment){
		...
		$resultat = $this->db->prepare($sql);
		$affectedLines = $resultat->execute(["id"=>$idComment]);
	    $resultat->closeCursor();
 		return $affectedLines;
    }   
